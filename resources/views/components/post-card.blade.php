...
@php
    ...
    $subtitle = $isModel ? $post->subtitle : ($post['subtitle'] ?? '');
    $excerpt = $isModel ? $post->excerpt : ($post['excerpt'] ?? '');
    $category = $isModel ? $post->category?->name : ($post['category'] ?? 'Uncategorized');
    $views = $isModel ? $post->formatted_views : ($post['views'] ?? 0);
    $wordCount = $isModel ? $post->formatted_word_count : ($post['word_count'] ?? 0);
    $readTime = $isModel ? $post->formatted_read_time : ($post['read_time'] ?? '0 min');
    $date = $isModel ? $post->published_at?->format('M d, Y') : ($post['date'] ?? '');
    $image = $isModel ? ($post->featured_image ? Storage::url($post->featured_image) : 'https://picsum.photos/seed/'.$post->id.'/800/600') : ($post['image'] ?? 'https://picsum.photos/seed/'.rand(1,100).'/800/600');
    $url = $isModel ? route('post.show', $post) : '#';
@endphp
...
<article class="glass-panel rounded-xl overflow-hidden hover:border-sf-accent/50 transition duration-300 group flex flex-col md:flex-row h-full">
    ...
    <div class="relative w-full md:w-1/3 h-48 md:h-auto overflow-hidden shrink-0">
        <a href="{{ $url }}">
            <img src="{{ $image }}" alt="{{ $title }}" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
        </a>
    ...
    <div class="p-6 flex flex-col flex-grow relative">
        ...
        <div class="relative z-10 flex flex-col h-full">
            ...
            <h3 class="text-sm text-sf-text-muted mb-3 font-medium">{{ $subtitle }}</h3>
