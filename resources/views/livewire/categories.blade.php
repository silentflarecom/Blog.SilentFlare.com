    ...
    <div class="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        @forelse($categories as $category)
            <a href="#" class="glass-panel p-6 rounded-xl hover:border-sf-accent/50 transition duration-300 group relative overflow-hidden block">
                <div class="absolute -right-6 -top-6 w-24 h-24 rounded-full blur-2xl transition duration-500" style="background-color: {{ $category->color ?? '#00f0ff' }}20;"></div>
                ...
                <div class="flex items-center justify-between mt-auto pt-4 border-t border-sf-border/30">
                    <span class="text-xs font-mono text-sf-text-muted">{{ $category->posts_count }} Articles</span>
        ...
        @empty
            <div class="col-span-full glass-panel rounded-xl p-8 text-center">
                <p class="text-sf-text-muted">No categories yet.</p>
            </div>
        @endforelse
    </div>
</div>
